...
export default function WelcomePopup({ visible = true, onClose = () => {} }) {
  ...
  return (
    <div aria-modal="true" role="dialog" style={{ position: 'fixed', inset: 0, background: 'linear-gradient(180deg, rgba(0,0,0,0.5), rgba(0,0,0,0.65))',display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: 20}}>
      <div style={{ width: 720, maxWidth: '96%', borderRadius: 12, overflow: 'hidden', display: 'flex', boxShadow: '0 12px 40px rgba(0,0,0,0.5)' }}>
        <div style={{ flex: 1, background: 'linear-gradient(135deg,#ffb4a2,#e63946)', padding: 24, color: '#fff' }}>
          ...
          <p style={{ marginTop: 12, fontSize: 13, lineHeight: 1.4 }}>Order mouth-watering meals, track delivery progress in real time and confirm receipt when delivered.</p>
